...
const BlockchainRequestors = {
    ...
    queryBlocksMsg(fromIndex, limit, offset, hideEmptyBlocks, contractAddress) {
        limit = typeof limit === 'undefined' ? 1 : limit;
        return { type: MessageType.QUERY_ALL, data: typeof fromIndex === 'undefined' ? 0 : fromIndex, limit, offset, hideEmptyBlocks, contractAddress }
    }
};
...
function updateIncommingBlocks(block, rawBlock, candy) {
    const searchParams = new SearchParams();

    const blockData = JSON.parse(block);
    if (blockData.type === 'Empty' && hideEmptyBlocks && searchParams.get('page', default_page) != '1') {
        return;
    }

    if (!lastestBlocks.find(_ => _.id === rawBlock.index)) {
...
function Candy(nodeList) {
            ...
            try {
                ...
                for (let a in blocks) {
                    ...
                    if(typeof that._resourceQueue[block.index] !== 'undefined') {
                        ...
                        updateIncommingBlocks(block.data, block, that);
                    }
                }
            } catch (e) {
            }
            lastestBlocks.sort((b1, b2) => b2.id - b1.id);
     ...
     * @param {Number} blockId
     * @param {Number} limit
     * @param {Boolean} hideEmptyBlocks
     */
    this.loadBlocks = function (blockId, limit=1, offset=0, hideEmptyBlocks=false, contractAddress) {
        let message = BlockchainRequestors.queryBlocksMsg(blockId, limit, offset, hideEmptyBlocks, contractAddress);
        that.broadcast(JSON.stringify(message));
    }
